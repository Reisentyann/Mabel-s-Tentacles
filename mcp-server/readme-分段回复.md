...
AstrBot 接入 QQ 官方 API 后，原版分段回复能力可能失效。本功能基于 MCP 服务器的文件写入行为实现“分段回复”：模型调用 `segmented_reply` 工具后，服务器会把一段长回复拆成多段，并按顺序写入 `data/segmented_replies/` 目录下的多个文本文件。

如果你的 AstrBot/上层适配器会把 MCP 写文件结果作为可见回复处理，那么每个分段文件写入动作就可以触发多次回复效果。
...
分段词、字数阈值、间隔时间和内容过滤器都从本地配置文件读取，不再需要每次调用工具时传入。
...
这个功能只负责在 MCP 服务器侧拆分文本并写入多个分段文件。最终能否表现为 QQ 中的多次回复，取决于 AstrBot 或上层 MCP 调用链是否会把这些文件写入结果转化为实际消息发送。
