<template>
  <div class="command-detail-container">
    <div class="header">
      <router-link to="/files">&larr; Back to Files</router-link>
    ...
    <div v-else-if="command" class="detail-card">
      <div class="meta-info">
        ...
        <p>
          ...
          {{ command.exit_code !== null ? command.exit_code : "N/A" }}
        </p>
        <p>
          <strong>Created:</strong>
          {{ new Date(command.created_at).toLocaleString() }}
        </p>
        <p v-if="command.finished_at">
          <strong>Finished:</strong>
          {{ new Date(command.finished_at).toLocaleString() }}
        </p>
      </div>
      ...
      <div class="code-section" v-if="command.result">
        <div class="section-header">
          <h3>Output (stdout)</h3>
          <button
            @click="downloadText(command.result, 'stdout.txt')"
            class="download-btn"
          >
            Download Output
          </button>
      ...
      <div class="code-section error-section" v-if="command.error_message">
        <div class="section-header">
          <h3>Error (stderr)</h3>
          <button
            @click="downloadText(command.error_message, 'stderr.txt')"
            class="download-btn"
          >
            Download Error
          </button>
