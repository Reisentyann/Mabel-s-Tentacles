...
| 功能 | 状态 |
|---|---|
| 文件内容解析并分类 | 描述引擎 v1 已上线（确定性事实字段）；语义分类待 llm 轨 |
| 字段索引机 | 规划中（attributes 检索提速：GIN 索引兜底，高频字段按"检索档位"（exact 等值 / range 范围）建表达式索引；档位登记进字段字典，与 HTTP `?attr.` 参数映射配对） |
| 文件管理机 | 规划中（文件生命周期编排层：显式移动 + 谱系边、短期下载票据、T2/T3 更新回填、盘库对账；DB 交互归 repo 层；自动只巡检报告，动手走 `move_file`） |
| 存量文件回填 | 已设计（版本号 + 幂等重分析），待实现 |
