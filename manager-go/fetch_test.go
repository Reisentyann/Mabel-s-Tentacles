...
// putRef 测试便捷：往 fakeStore 塞一条取件事实（uuid ↔ 相对路径）。
...
func TestLocate(t *testing.T) {
	...
	writeFile(t, dir, "a/有.txt", "x") // Locate 不做盘上检查，但顺手铺盘面
...
func TestOpenSemantics(t *testing.T) {
	...
	writeFile(t, dir, "正常.txt", "梅贝尔的触手书房")
...
func TestOpenBufferHitAndInvalidate(t *testing.T) {
	m, st, _, dir := newTestManager(t)
	putRef(st, "u", "buf.txt", false)
	abs := filepath.Join(dir, "buf.txt")
	writeFile(t, dir, "buf.txt", "v1-content")

	if _, err := m.Open(context.Background(), "u"); err != nil {
		t.Fatal(err)
	}
	...
	of, err := m.Open(context.Background(), "u")
	if err != nil {
		t.Fatal(err)
	}
	of.Content.Close()
	...
	rf, err := m.Read(context.Background(), "u", 0)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReadLimit(t *testing.T) {
	m, st, _, dir := newTestManager(t)
	putRef(st, "u", "limit.txt", false)
	writeFile(t, dir, "limit.txt", "0123456789")

	rf, err := m.Read(context.Background(), "u", 4)
	if err != nil {
		t.Fatal(err)
	}
